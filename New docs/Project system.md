...
**Workbook projects** are defined inside a particular workbook and serve to automate the workbook, and to define functions that are specific to the containing workbook. Workbook project code as well as the output files are stored inside the workbook itself.
